  ...
  <ion-page>
    <ion-header>
      <ion-toolbar>
        ...
        <ion-title>Distribusi Steril</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content :fullscreen="true">
      <div class="content-container">
        ...
        <ion-card>
          <ion-card-header>
            <ion-card-title>Pilih Instrumen</ion-card-title>
          </ion-card-header>
          <ion-card-content>
            <ion-list>
              <ion-item v-for="instrument in availableInstruments" :key="instrument.id">
                <ion-label>
                  <h3>{{ instrument.name }}</h3>
                  <p>{{ instrument.code }} - Tersedia: {{ instrument.available_stock }}</p>
                </ion-label>
                <ion-input
                  v-model.number="selectedItems[instrument.id]"
                  type="number"
                  :min="0"
                  :max="instrument.available_stock"
                  placeholder="0"
                ></ion-input>
              </ion-item>
            </ion-list>
            ...
            <ion-button
              ...
              class="submit-button"
            >
              {{ loading ? 'Membuat Transaksi...' : 'Buat Transaksi' }}
            </ion-button>
          </ion-card-content>
        </ion-card>
      </div>
...
const fetchUnitAndInstruments = async (unitId: string) => {
  try {
    ...
    const instrumentsResponse = await api.get('/instruments', {
      params: { available_for_steril: true }
    })
    availableInstruments.value = instrumentsResponse.data.data
  } catch (error: any) {
...
const createTransaction = async () => {
  ...
  try {
    loading.value = true
    const response = await api.post('/transactions/create-steril', {
    ...
    const alert = await alertController.create({
      header: 'Sukses',
      message: 'Transaksi berhasil dibuat. QR Code telah digenerate.',
      buttons: [
        {
          text: 'Lihat QR',
          handler: () => {
